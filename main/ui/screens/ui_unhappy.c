...
static const char *TAG = "ui_unhappy";

lv_obj_t *ui_Unhappy;

static lv_obj_t *img_unhappy = NULL;
...
static void update_button_colors(lv_obj_t *selected_btn)
{
    ...
    // {
    //     _ui_screen_change(&ui_Unhappy, LV_SCR_LOAD_ANIM_NONE, 0, 0,
    //                       ui_Unhappy_screen_init);
    // }
...
{
    _ui_screen_delete(&ui_Unhappy);
}

void ui_Unhappy_screen_init(void)
{
    ui_Unhappy = lv_obj_create(NULL);

    lv_obj_clear_flag(ui_Unhappy, LV_OBJ_FLAG_SCROLLABLE); /// Flags
    lv_obj_set_style_bg_color(ui_Unhappy, lv_color_hex(0x000000), 0);

    lv_obj_t *ui_LabelMenu = lv_label_create(ui_Unhappy);
    // lv_label_
    lv_obj_align(ui_LabelMenu, LV_ALIGN_TOP_MID, 0, 38);
    lv_obj_set_style_text_font(ui_LabelMenu, &MiSansRegular_40, 0);
    lv_label_set_text(ui_LabelMenu, "很遗憾 你输了");
    lv_obj_set_style_text_color(ui_LabelMenu, lv_color_hex(0xFFAA00), 0);

    img_unhappy = lv_img_create(ui_Unhappy);
    lv_img_set_src(img_unhappy, &unhappy_image);
    lv_obj_align(img_unhappy, LV_ALIGN_TOP_LEFT, 164, 101);
    lv_obj_add_flag(img_unhappy, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(img_unhappy, button_click_handler, LV_EVENT_SHORT_CLICKED, NULL);

    white_btn = lv_btn_create(ui_Unhappy);
    ...
    lv_obj_t *ui_LabelButton1 = lv_label_create(white_btn);
    // lv_label_
    lv_obj_align(ui_LabelButton1, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_text_font(ui_LabelButton1, &MiSansBold_26, 0);
    lv_label_set_text(ui_LabelButton1, "取消");
    lv_obj_set_style_text_color(ui_LabelButton1, lv_color_hex(0xFFFFFF), 0);

    white_btn2 = lv_btn_create(ui_Unhappy);
    ...
    lv_obj_add_event_cb(ui_Unhappy, event_unload_handler, LV_EVENT_SCREEN_UNLOADED,
                        NULL);
}