...
static const char *TAG = "ui_lights";
...
void ui_Lights_screen_init(void)
{
    ...
    lv_obj_set_style_bg_color(ui_Lights, lv_color_hex(0x171726), 0);

    lv_obj_t *ui_LabelMenu = lv_label_create(ui_Lights);
    // lv_label_
    lv_obj_align(ui_LabelMenu, LV_ALIGN_TOP_MID, 0, 29);
    lv_obj_set_style_text_font(ui_LabelMenu, &MiSansBold_26, 0);
    lv_label_set_text(ui_LabelMenu, "屏幕亮度");
    lv_obj_set_style_text_color(ui_LabelMenu, lv_color_hex(0xffffff), 0);

    img_home = lv_img_create(ui_Lights);
    ...
    img_leftarr = lv_img_create(ui_Lights);
    ...
    // 添加滑轨（slider）
    lv_obj_t *ui_SliderVolume = lv_slider_create(ui_Lights);
    ...
    img_volume = lv_img_create(ui_Lights);
    lv_img_set_src(img_volume, &lights);
    ...
    lv_obj_add_event_cb(ui_Lights, event_unload_handler, LV_EVENT_SCREEN_UNLOADED,
                        NULL);
}